...
module.exports = function () {
    ...
    this.checkUnauthorizedRequest = function (req, res) {
        if (req.headers.host != 'localhost:' + this.port) {
            console.log("Returning error 401 to request")
    ...
    this.initRoutes = function (self) {
        ...
        /*--------------------*/
        self.router.get('/blinds/:id/open', function (req, res) {
            console.log("[GET] api/blinds/:id/open - " + JSON.stringify(req.headers) + " -> " + JSON.stringify(req.body))
            let blindIndex = parseInt(req.params.id, 10)
            self.controller.handleOpenBlind(blindIndex, res)
        })
        self.router.get('/blinds/:id/close', function (req, res) {
            console.log("[GET] api/blinds/:id/close - " + JSON.stringify(req.headers) + " -> " + JSON.stringify(req.body))
            let blindIndex = parseInt(req.params.id, 10)
            self.controller.handleCloseBlind(blindIndex, res)
        })
        self.router.get('/blinds/:id/setpos/:pos', function (req, res) {
            console.log("[GET] api/blinds/:id/setpos/:pos - " + JSON.stringify(req.headers) + " -> " + JSON.stringify(req.body))
            let blindIndex = parseInt(req.params.id, 10)
            let pos = parseInt(req.params.pos, 10)
            self.controller.handleSetBlindPos(blindIndex, pos, res)
        })
        self.router.get('/blinds/:id/toggle', function (req, res) {
            console.log("[GET] api/blinds/:id/toggle - " + JSON.stringify(req.headers) + " -> " + JSON.stringify(req.body))
            let blindIndex = parseInt(req.params.id, 10)
            self.controller.handleToggleBlind(blindIndex, res)
        })
        self.router.get('/blinds/:id/getpos', function (req, res) {
            console.log("[GET] api/blinds/:id/getpos - " + JSON.stringify(req.headers) + " -> " + JSON.stringify(req.body))
            let blindIndex = parseInt(req.params.id, 10)
        ...
        self.router.get('/blinds/:id/stop', function (req, res) {
            ...
            let blindIndex = parseInt(req.params.id, 10)
            self.controller.handleStopBlind(blindIndex, res)
        })
        /*--------------------*/
